...
namespace Ultra.Xpo
{
    public sealed class UltraViewRecord : ICustomTypeDescriptor
    {
        ...
        public object this[CriteriaOperator property]
        {
            get
            {
                UltraViewProperty prop = View.Properties[property];
                if (prop == null || !prop.Fetch)
                    throw new ArgumentException("Fix error message");
                //throw new ArgumentException(Res.GetString(Res.View_PropertyNotFetched, CriteriaOperator.ToString(property)));
                return this[prop.Name];
            }
        }
        public UltraViewRecord(UltraXPView view, object[] data)
        {
            this.view = view;
            this.data = data;
        }
    ...
    sealed class ViewPropertyDescriptor : PropertyDescriptor
    {
        ...
        {
            return ((UltraViewRecord)component).Data[index];
        }
    ...
    public class UltraViewProperty : IDataColumnInfoProvider
    {
        string name;
        ...
        bool fetch;
        UltraXPView owner;
        protected internal void SetOwner(UltraXPView owner)
        {
            this.owner = owner;
        }
        void ResetView()
        ...
        [Editor("DevExpress.Xpo.Design.XPViewExpressionEditor, " + AssemblyInfo.SRAssemblyXpoDesignFull, "System.Drawing.Design.UITypeEditor")]
        [Category("Data")]
        ...
        public UltraViewProperty()
        {
            sorting = SortDirection.None;
            fetch = true;
            name = string.Empty;
        }
        public UltraViewProperty(string name, SortDirection sorting, CriteriaOperator property, bool group, bool fetch)
        {
            this.name = name;
        ...
        public UltraViewProperty(string name, SortDirection sorting, string property, bool group, bool fetch)
        ...
        {
            readonly UltraViewProperty column;
            public ViewPropertyIDataPropertyInfoWrapper(UltraViewProperty column)
            {
                this.column = column;
            }
    ...
    class ViewPropertyConverter : TypeConverter
    {
        ...
        {
            UltraViewProperty prop = val as UltraViewProperty;
            if (destinationType == typeof(InstanceDescriptor) && prop != null)
            {
                ConstructorInfo ctor = typeof(UltraViewProperty).GetConstructor(new Type[] { typeof(string), typeof(SortDirection), typeof(string), typeof(bool), typeof(bool) });
                if (ctor != null)
                {
    ...
    {
        UltraXPView owner = null;
        public ViewPropertiesCollection(UltraXPView owner)
            : base()
        {
            this.owner = owner;
        }
        public void Add(UltraViewProperty sortProperty)
        {
            ((IList)this).Add(sortProperty);
        }
        public void AddRange(UltraViewProperty[] sortProperties)
        {
            foreach (UltraViewProperty sp in sortProperties)
                Add(sp);
        }
        ...
        {
            ((UltraViewProperty)value).SetOwner(owner);
            if (owner != null)
                owner.ClearProps();
        }
        protected override void OnRemoveComplete(int index, object value)
        {
            ((UltraViewProperty)value).SetOwner(null);
            if (owner != null)
                owner.ClearProps();
        }
        public UltraViewProperty this[int index] { get { return (UltraViewProperty)List[index]; } }
        public UltraViewProperty this[CriteriaOperator expression]
        {
            get
            {
                foreach (UltraViewProperty prop in this)
                {
        ...
        public UltraViewProperty this[string name]
        {
            get
            {
                foreach (UltraViewProperty prop in this)
    ...
    [System.Drawing.ToolboxBitmap(typeof(UltraXPView))]
#endif
    public class UltraXPView : Component, ISupportInitialize, IBindingList, ITypedList, IFilteredXtraBindingList, IXPClassInfoAndSessionProvider
    {
        bool selectDeleted;
        Session session;
        XPClassInfo info;
        ...
        List<object> GetSampleData()
        {
            ...
            list.Add(new UltraViewRecord(this, new object[DisplayProps.Count]));
            return list;
        }
        internal void Clear()
        {
                    ...
                    , new AsyncSelectDataCallback(delegate (List<object[]> data, Exception ex)
                    {
                        ...
                        if (ex == null)
                        {
                            ...
                            foreach (object[] rec in data)
                                list.Add(new UltraViewRecord(this, rec));
                            objects = list;
                            Reset(false);
                            if (callback != null)
                            {
        ...
        public SelectStatement GenerateSelectStatement()
        {
            CriteriaOperatorCollection properties;
            CriteriaOperatorCollection groupProperties;
            SortingCollection sorting;
            PrepareLoad(out properties, out groupProperties, out sorting);
            return GenerateSelectStatementCore(properties, groupProperties, sorting);
        ...
        void PrepareLoad(out CriteriaOperatorCollection properties, out CriteriaOperatorCollection groupProperties, out SortingCollection sorting)
        {
            ...
            foreach (UltraViewProperty prop in Properties)
            {
                if (prop.Fetch)
            ...
            if (properties.Count == 0)
                throw new InvalidOperationException("Fix error message");
            //throw new InvalidOperationException(Res.GetString(Res.View_AtLeastOneFetchProperptyShouldBeDefined));
        }
        List<object> EndLoad(CriteriaOperatorCollection properties, CriteriaOperatorCollection groupProperties, SortingCollection sorting)
        {
            ...
            //DevExpress.Xpo.Generators.ClientSelectSqlGenerator.GenerateSelect(ObjectClassInfo,criteria,properties,sorting,groupProperties,groupCriteria,null,skipReturnedRecords,topReturnedRecords);
            List<object> list = new List<object>();
            foreach (object[] rec in data)
                list.Add(new UltraViewRecord(this, rec));
            return list;
        }
        public SelectStatement GenerateSelectStatementCore(CriteriaOperatorCollection properties, CriteriaOperatorCollection groupProperties, SortingCollection sorting)
        {
            ...
            return DevExpress.Xpo.Generators.ClientSelectSqlGenerator.GenerateSelect(ObjectClassInfo, criteria, properties, sorting, groupProperties, groupCriteria, null, skipReturnedRecords, topReturnedRecords);
           
        }
        public void Reload()
        {
            if (isInAsyncLoading) throw new InvalidOperationException("Fix error message");
            //if (isInAsyncLoading) throw new InvalidOperationException(Res.GetString(Res.View_View_IsInLoading));
            Clear();
            Reset(false);
        }
        public UltraViewRecord this[int index]
        {
            get
            {
                return (UltraViewRecord)Objects[index];
            }
        }
        ...
        public XPClassInfo ObjectClassInfo
        {
            ...
            set
            {
                if (!this.IsDesignMode && !this.Initializing)
                    ...
                    throw new NotSupportedException("Fix Message");
        ...
        public Type ObjectType
        {
            ...
            {
                if (!this.IsDesignMode && !this.Initializing)
                    throw new NotSupportedException("Fix Error Message");
        ...
        public UltraXPView(IContainer container)
            : this()
        {
            container.Add(this);
        }
        public UltraXPView(Session session, XPClassInfo info, CriteriaOperatorCollection properties, CriteriaOperator criteria)
            : this(session, info, properties, criteria, null)
        {
        }
        public UltraXPView(Session session, XPClassInfo info, CriteriaOperatorCollection properties, CriteriaOperator criteria, CriteriaOperator groupCriteria)
            : this()
        {
        ...
        public UltraXPView(Session session, Type objType)
            : this(session, session.GetClassInfo(objType))
        {
        }
        public UltraXPView(Session session, XPClassInfo info)
            : this(session, info, String.Empty, null)
        {
        }
        public UltraXPView(Session session, Type objType, CriteriaOperatorCollection properties, CriteriaOperator criteria)
            : this(session, session.GetClassInfo(objType), properties, criteria)
        {
        }
        public UltraXPView(Session session, Type objType, string properties, CriteriaOperator criteria)
            : this(session, session.GetClassInfo(objType), properties, criteria)
        {
        }
        public UltraXPView(Session session, XPClassInfo info, string properties, CriteriaOperator criteria)
            : this(session, info, GetProperties(info, properties), criteria)
        {
        }
        public UltraViewProperty AddProperty(CriteriaOperator property)
        {
            string name;
        ...
        public UltraViewProperty AddProperty(string property)
        {
            return AddProperty(CriteriaOperator.Parse(property));
        }
        public UltraViewProperty AddProperty(string name, string property)
        {
            return AddProperty(name, property, false);
        }
        public UltraViewProperty AddProperty(string name, CriteriaOperator property)
        {
            return AddProperty(name, property, false);
        }
        public UltraViewProperty AddProperty(string name, string property, bool group)
        {
            return AddProperty(name, CriteriaOperator.Parse(property), group, true, SortDirection.None);
        }
        public UltraViewProperty AddProperty(string name, CriteriaOperator property, bool group)
        {
            return AddProperty(name, property, group, true, SortDirection.None);
        }
        public UltraViewProperty AddProperty(string name, string property, bool group, bool fetch, SortDirection sorting)
        {
            return AddProperty(name, CriteriaOperator.Parse(property), group, fetch, sorting);
        }
        public UltraViewProperty AddProperty(string name, CriteriaOperator property, bool group, bool fetch, SortDirection sorting)
        {
            UltraViewProperty prop = new UltraViewProperty(name, sorting, property, group, fetch);
            Properties.Add(prop);
            Reset(true);
            return prop;
        }
